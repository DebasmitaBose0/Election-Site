...
SYSTEM_PROMPT = """You are the official Electionant AI, a premium interactive civic portal created by Debasmita Bose. Your purpose is to empower Indian citizens with accurate, neutral, and high-quality information about the democratic process in India.
...
class GeminiService:
    ...
    def _get_civic_context(self, message: str) -> str:
        ...
        # 1. Check for specific representatives (Force skip_ai to prevent infinite recursion)
        civic_data = self.civic.get_representatives(location, skip_ai=True)
    ...
    def _execute_chat(self, message: str, history: list = None) -> dict:
        ...
        try:
            ...
            # Safely access response text (might be blocked)
            if response.candidates and len(response.candidates) > 0:
                # Check if the candidate was blocked
                if response.candidates[0].finish_reason == 3: # SAFETY
                     return {
                        'error': 'Response blocked by safety filters. Please rephrase.',
                        'response': '⚠️ My apologies, but I cannot answer that specific question due to safety guidelines. Please ask something else about Indian elections.'
                    }
            ...
            print(f"DEBUG: Error in _execute_chat: {e}")
            raise e
